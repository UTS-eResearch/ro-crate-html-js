...
class Preview {
    ...
    async renderMetadataForItem(id) {
        ...
        if (types.includes("Dataset") || types.includes("File") ||types.includes("ImageObject")) {
            var path = item["@id"];
  ...
  addMetadataRow(target, item, prop){
    ...
    var types = this.crate.utils.asArray(item["@type"]);
    if (prop === "@id") {
        if (types.includes("Dataset") || types.includes("File") || types.includes("ImageObject") || item["@id"].match(/^https?:\/\//i)) {
