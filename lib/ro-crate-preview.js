...
class Preview  {
    constructor(crate) {
        ...
        const c = this;
        this.crate.resolveContext().then(function(){c.addGloss()});
    } 
    ...
    async addGloss() {
        /// NO LONGER USED - LEAVING HERE COS THERE ARE TODOs
        if (this.crate.context) {
            const propEls = document.getElementsByClassName("prop");
            console.log(propEls)
            for (let p of propEls) {
                    var propName = p.innerHTML;
                    const href = this.crate.resolveTerm(propName);
                    console.log(href)
                    if (href) {
                        p.innerHTML = `${propName}<span>&nbsp;</span><a href="${href}">[?]</a>`;
                    }
...
/*
            function () {
                ...
                // TODO this should be in the resolve term method
...
*/
  ...
  metadataRow(item, prop){
    return `<tr>
            <th style="text-align:left;" class="prop">${prop}</th>
