...
const displayTypeTemplates = {
    ...
    "GeoCoordinates" : (item)=> {return `Lat: ${item.latitute }, Long: ${item.longitude}`}
}
...
class Preview  {
   ...
   displayPlaces() {
        ...
        // Places is an array
        if (config && this.places && this.places.length > 0) {
            var jsonFile = GeoJSON.parse(this.places, {Point: ['latitude', 'longitude']})
