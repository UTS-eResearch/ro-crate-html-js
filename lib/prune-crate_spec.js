...
const prune = require('./prune-crate');
...
describe('Pruning', function () {
    it('Does basic pruning', function () {
        ...
        const config1 = {
            "types": 
               {"Person" : {
                 "props" :  {"birthPlace": null}
               }
            }
        }   
        
        const pruned1 = prune(item1, crate, config1);
        assert.equal(pruned1.getItem("#Place1").name, "Place 1");
        assert.equal(pruned1.getGraph().length, 4);

        config1.types.Person.props = {"birthPlace": {"types":  {"Place": {"props": {"location" : null}}}}};
        const pruned2 = prune(item1, crate, config1);
        assert.equal(pruned2.getItem("#Location1").name, "Location 1");
        assert.equal(pruned2.getGraph().length, 5);
     

        config1.types["Person"].reverseProps =  {"about": null};
        item1["@reverse"] = {"about": {"@id": doc1["@id"]}}
        const pruned3 = prune(item1, crate, config1);
