...
class DisplayableProp {
    constructor(graph, prop, value, config, displayed, root){
        ...
        this.external = false;
        this.displayedItems = displayed || {};
        this.displayableValues = [];
        const values = asArray(value);

        for (let val of values) {
            this.displayableValues.push(new DisplayableValue(graph, val, this.config, this.displayedItems, prop, this.rootDisplayableItem));
        }
        this.displayableValues.sort((a, b) => (a.text > b.text) ? 1 : -1)

        this.paginatedValues = new Page({
            values: this.displayableValues,
